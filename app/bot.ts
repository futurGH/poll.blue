import { default as Agent, AppBskyNotificationListNotifications, AppBskyFeedPost, AppBskyFeedDefs } from "https://esm.sh/v115/@atproto/api@0.12.2"
...
export class Bot {
    ...
    async postPoll(poll: Poll, replyRef: AppBskyFeedPost.ReplyRef, author: string): Promise<PostPollResult | undefined> {
        ...
        const { text, links, pollFacets } = generatePollText({ visibleId, poll, replyRef, author, pollStyle: 'bot' }, this.agent!);
        await Promise.all([
